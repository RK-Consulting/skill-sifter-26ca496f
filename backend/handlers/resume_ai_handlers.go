...
// ollamaBaseURL resolves the endpoint from the environment. In Docker, the
// recommended value is host.docker.internal so the backend can reach an Ollama
// instance running on the host. OLLAMA_URL always wins when explicitly set.
func ollamaBaseURL() string {
    if value := strings.TrimSpace(os.Getenv("OLLAMA_URL")); value != "" { return strings.TrimRight(value, "/") }
    if os.Getenv("DOCKER_ENV") == "true" || os.Getenv("DOCKER_ENV") == "1" { return "http://host.docker.internal:11434" }
    return "http://127.0.0.1:11434"
}

func ollamaModel() string { return db.GetEnv("OLLAMA_MODEL", "llama3.1:8b") }
...
func callOllama(text string) (resumeAIResult,string) {
    ...
    payload:=map[string]interface{}{"model":ollamaModel(),"prompt":prompt,"stream":false,"format":"json","options":map[string]interface{}{"temperature":0}}
    body,_:=json.Marshal(payload);client:=&http.Client{Timeout:90*time.Second};resp,err:=client.Post(ollamaBaseURL()+"/api/generate","application/json",bytes.NewReader(body));if err!=nil{return resumeAIResult{},"Ollama unavailable at "+ollamaBaseURL()+": "+err.Error()};defer resp.Body.Close();if resp.StatusCode>=300{b,_:=io.ReadAll(io.LimitReader(resp.Body,4096));return resumeAIResult{},fmt.Sprintf("Ollama returned %d: %s",resp.StatusCode,string(b))};var out struct{Response string `json:"response"`};if err:=json.NewDecoder(resp.Body).Decode(&out);err!=nil{return resumeAIResult{},"Invalid Ollama response: "+err.Error()};var parsed resumeAIResult;if err:=json.Unmarshal([]byte(out.Response),&parsed);err!=nil{return resumeAIResult{},"Ollama JSON parse failed: "+err.Error()};return parsed,""
}
...
func GetResumeHealth(w http.ResponseWriter,r *http.Request){url:=ollamaBaseURL();client:=&http.Client{Timeout:5*time.Second};resp,err:=client.Get(url+"/api/tags");if err!=nil{respondWithJSON(w,200,models.ApiResponse{Success:true,Message:"Ollama is not reachable",Data:map[string]interface{}{"available":false,"url":url,"model":ollamaModel(),"error":err.Error()}});return};defer resp.Body.Close();respondWithJSON(w,200,models.ApiResponse{Success:true,Message:"Ollama health checked",Data:map[string]interface{}{"available":resp.StatusCode<300,"url":url,"model":ollamaModel()}})}
