...
func seedFixtures(t *testing.T, db *sql.DB, tenantID string) fixtures {
	t.Helper()
	var f fixtures
	db.QueryRow(`INSERT INTO users (username, email, password, role, tenant_id, company_name) VALUES ($1, $2, 'x', 'recruiter', $3, $3) RETURNING id`,
		"user_"+tenantID, tenantID+"user@test.com", tenantID).Scan(&f.userID)
	db.QueryRow(`INSERT INTO candidates (name, email, tenant_id, company_name, status) VALUES ('Test Candidate', $1, $2, $2, 'active') RETURNING id`,
		tenantID+"cand@test.com", tenantID).Scan(&f.candidateID)
	var clientID int
	db.QueryRow(`INSERT INTO clients (name, tenant_id) VALUES ('Test Client', $1) RETURNING id`, tenantID).Scan(&clientID)
	db.QueryRow(`INSERT INTO requirements (client_id, title, tenant_id) VALUES ($1, 'Test Req', $2) RETURNING id`, clientID, tenantID).Scan(&f.requirementID)
	return f
}
